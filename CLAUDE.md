...
AMI ID 由 SSM Parameter `/precis/processor-ami-id` 管理（Image Builder 更新後手動 `aws ssm put-parameter`）。EBS 35GB。
...
- **AMI**：SSM `/precis/processor-ami-id`（AL2023 ECS GPU + ML stack）
